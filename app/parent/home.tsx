    ...
    () => [
      {
        ...
        render: () =>
          !nextAppt ? (
            ...
            <Card onPress={() => router.push(`/parent/appointment/${nextAppt.id}` as any)}>
      ...
      },
      {
        ...
        render: () => (rx.length === 0 ? <Empty text="No prescriptions yet." /> : rx.slice(0, 3).map((r) => <Row key={r.id} primary={r.drug} secondary={r.status} />)),
      },
      {
        key: "orders",
        title: "Recent Orders",
...
/** tiny helpers stay here or can be moved to shared later */
